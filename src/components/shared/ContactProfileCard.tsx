...
export function ContactProfileCard({ profile, delay = 0 }: { profile: ContactProfileData; delay?: number }) {
  return (
    <Reveal delay={delay}>
      <div className="flex h-full flex-col items-center rounded-xl border border-border bg-white p-6 text-center shadow-sm">
        ...
        <img
          src={profile.photoUrl || `https://api.dicebear.com/9.x/personas/svg?seed=${profile.avatarSeed}`}
          alt=""
          width={72}
          height={72}
          loading="lazy"
          className="size-[72px] rounded-full bg-cream object-cover"
        />
